...
public class SearchServiceImpl implements SearchService {
    ...
    public List<SearchDto> allSiteSearch(String searchText, int offset, int limit) {
        ...
        for (LemmaEntity l : foundLemmaList) {
            if (l.getLemma().equals(searchText) || textLemmaList.contains(l.getLemma())) {
    ...
    private List<SearchDto> getSearchData(HashMap<PageEntity, Float> pageList, List<String> textLemmaList) {
        List<SearchDto> result = new ArrayList<>();
    ...
    private List<SearchDto> getSearchDtoList(List<LemmaEntity> lemmaList, List<String> textLemmaList, int offset, int limit) {
        ...
        if (lemmaList.size() >= textLemmaList.size()) {
            ...
            LinkedHashMap<PageEntity, Float> sortedPageByAbsRelevance = getPageAbsRelevance(foundPageList, foundIndexList);
    ...
    private LinkedHashMap<PageEntity, Float> getPageAbsRelevance(List<PageEntity> pageList, List<IndexEntity> indexList) {
        ...
        return pageWithAbsRelevance.entrySet().stream().sorted(Map.Entry.comparingByValue(Comparator.reverseOrder())).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (e1, e2) -> e1, LinkedHashMap::new));
    }
}
